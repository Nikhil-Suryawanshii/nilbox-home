...
import '../../../../models/eCommerce/cart/cart_product.dart';
import '../components/cart_colour.dart';
...
class _EcommerceMyCartLayoutState extends ConsumerState<EcommerceMyCartLayout> {
                          ...
                          child: Column(
                            ...
                            children: AnimationConfiguration.toStaggeredList(
                              ...
                              children: [
                                ...
                                // Gap(MediaQuery.of(context).size.height / 9.5),
                              ],
                            ),
                          ),
                        ),
  ...
  /// Updated AppBar to look like a "Floating White Pill"
  ...
  Widget _buildCartProductList({
    ...
    return Container(
      ...
      padding: EdgeInsets.fromLTRB(1, 0, 16, 0),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Visibility(
          ...
          ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              // (Assuming you want per-item selection, or check if the shop is selected)
              final isSelected = ref.watch(shopIdsProvider).contains(cartItem.shopId);

              return Padding(
                padding: EdgeInsets.only(bottom: 10.h), // Spacing between items
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: [
                    /// 1. SELECTION CIRCLE (Left Side)
                    GestureDetector(
                      onTap: () {
                        // Toggle selection logic here
                        // Example: Toggle shop ID or specific product ID
                        ref.read(shopIdsProvider.notifier).toggleShopId(cartItem.shopId);
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(
                            color: isSelected
                                ...
                                : colors(context).hintTextColor!, // Grey when unselected
                            width: 1.5,
                          ),
                        ),
                        child: isSelected
                    ...
                    /// 2. PRODUCT CARD (Expanded to fill width)
                    Expanded(
                      child: CartProductCard(
                        ...
                        onDelete: () async {
                          // 1. Check Multivendor Logic (Same as decrement)
                          if (!checkMultivendor()) {

                            // 2. Call addToCart with quantity 0 to remove item
                            await ref.read(cartController.notifier).onDeleteCartFun(
                                productId: cartItem.cartProduct[index].id,
                              // addToCartModel: AddToCartModel(
                              //   productId: cartItem.cartProduct[index].id,
                              //   quantity: 0, // 👈 Setting quantity to 0 deletes it
                              // ),
                            );

                            calculateCartSummery();
                          } else {
                            // 3. Multivendor Logic (Same as decrement)
                            if (ref.read(shopIdsProvider).isNotEmpty) {
                              ref.refresh(shopIdsProvider);
                              ref.refresh(cartSummeryController.notifier);
                            }
  ...
  Widget _buildPromoCodeApplyWidget({required BuildContext context}) {
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: Colors.white, // light peach bg
        ...
        child: Row(
          children: [
            /// LEFT ICON
            SvgPicture.asset(
              Assets.svg.discount, // coupon icon
              width: 30.w,
              height: 30.h,
              color: colors(context).primaryColor,
              // colorFilter:
              // const ColorFilter.mode(colors(context).primaryColor, BlendMode.srcIn),
            ),
  ...
  Widget _buildSummaryWidget({required BuildContext context}) {
    ...
    final summaryData = ref.watch(cartSummeryController);
    final totalAmount = summaryData['totalAmount'] ?? 0.0;
    final discount = summaryData['discount'] ?? 0.0;
    final deliveryCharge = summaryData['deliveryCharge'] ?? 0.0;
    final taxAmount = summaryData['orderTaxAmount'] ?? 0.0;
    final payableAmount = summaryData['payableAmount'] ?? 0.0;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.only(
          ...
          // If this is a standalone card and not a bottom sheet, you might want all corners rounded:
          // borderRadius: BorderRadius.circular(30.r),
        ),
        boxShadow: [
          BoxShadow(
      ...
      child: Column(
        ...
        mainAxisSize: MainAxisSize.min,
        children: [
          /// 1. Grey Drag Handle (Visual element from image)
          Container(
            width: 60.w,
            height: 4.h,
            decoration: BoxDecoration(
          ...
          /// 3. Delivery Charge (Shows "Free" if 0, matching implied logic)
          _buildSummaryRow(
            context: context,
            // If you want strictly "Delivery Free" label when free, you can add logic here
          ...
          /// 4. Tax Summary (Aggregated single line to match image)
          _buildSummaryRow(
            context: context,
            title: 'Tax Summary', // Ensure you have this string in your localization
            value: taxAmount,
            ref: ref,
          ),
          Gap(5.h),
      ...
      children: [
        Text(
          ...
          style: AppTextStyle(context).bodyText.copyWith(
            color: Colors.grey,
            fontSize: 14.sp,
            fontStyle: FontStyle.italic, // Matches image style
            fontWeight: FontWeight.w500,
          ),
        ),
        Text(
          isDelivery && (value == 0 || value == 0.0)
              ? "Free" // Or S.of(context).free
              : GlobalFunction.price(
            ref: ref,
              ...
              AbsorbPointer(
                ...
                child: SizedBox(
                  ...
                  child: CustomButton(
                    ...
                    // Assuming CustomButton has a style or color property
                    buttonColor: ref.watch(shopIdsProvider).isNotEmpty
                        ? colors(context).primaryColor
                        : ColorTween(
                            begin: colors(context).primaryColor,
                    ...
                    // Ensure text style inside CustomButton is white
                  ),
                ),
              ),
              Gap(10.h), // Safe area padding
            ],
          ),
        );
      },
  ...
  /// Helper widget to keep the main code clean and match the row style
  Widget _buildSheetRow1({
    required BuildContext context,
    required String title,
    required dynamic value,
    required WidgetRef ref,
