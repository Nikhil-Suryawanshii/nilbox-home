...
class ApiInterceptors {
  ...
  static String handleError(DioException exception) {
    ...
    switch (exception.type) {
      case DioExceptionType.receiveTimeout: errorMessage = 'Receive time out!'; break;
      case DioExceptionType.sendTimeout: errorMessage = 'Send time out!'; break;
      case DioExceptionType.connectionError: errorMessage = 'Connection error!'; break;
      default: 
  ...
  static void _handleUnauthorized() {
    GlobalFunction.showCustomSnackbar(message: 'Session expired. Please login again.', isSuccess: false);
    ...
    // Clear Seller Auth
    Box sellerAuthBox = Hive.box(AppConstants.sellerAuthBox);
    sellerAuthBox.delete(AppConstants.sellerAuthToken);

    GlobalFunction.navigatorKey.currentState?.pushNamedAndRemoveUntil(Routes.login, (route) => false);
  }
}