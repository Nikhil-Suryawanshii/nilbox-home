...
class ShopController extends StateNotifier<bool> {
  ...
  // Future<void> followUnfollowShop({
  //   required int shopId,
  // }) async {
  //   try {
  ...
  //     await ref
  //         .read(shopServiceProvider)
  //         .getShopFollowUnfollow(shopId: shopId);
  //
  //     state = false;
  //   } catch (error, stackTrace) {
  //     debugPrint(error.toString());
  ...
  Future<void> followUnfollowShop({
    ...
    if (index != -1) {
      _shops[index] = _shops[index].copyWith(
        isFollowed: !_shops[index].isFollowed,
      );
    }
...
// final shopFollowProvider =
// StateNotifierProvider.family<ShopFollowNotifier, bool, int>(
//       (ref, shopId) => ShopFollowNotifier(),
// );
//
// class ShopFollowNotifier extends StateNotifier<bool> {
//   ShopFollowNotifier() : super(false);
//
