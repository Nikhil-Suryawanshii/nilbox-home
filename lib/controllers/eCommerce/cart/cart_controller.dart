...
class CartController extends StateNotifier<CartState> {
  ...
  Future<void> onDeleteCartFun({required int productId}) async {
    try {
      // 1. Start loading
      state = CartState(isLoading: true, cartItems: cartItems);

      // 2. Call API (delete OR decrement-to-zero)
      final response = await ref
          .read(cartServiceProvider)
          .decrementQty(productId: productId);
      // 👉 replace with deleteCartItem() if you have it

      if (response.statusCode == 200) {
        // 3. LOCAL DELETE LOGIC (IMPORTANT PART)
        final List<CartItem> updatedCartItems = [];

        for (final cartItem in _cartItems) {
          // Remove product from this shop
          final updatedProducts = cartItem.cartProduct
              .where((product) => product.id != productId)
              .toList();

          // If shop still has products → keep it
          if (updatedProducts.isNotEmpty) {
            updatedCartItems.add(
              CartItem(
                shopId: cartItem.shopId,
                shopName: cartItem.shopName,
                shopLogo: cartItem.shopLogo,
                shopRating: cartItem.shopRating,
                hasGift: cartItem.hasGift,
                cartProduct: updatedProducts,
              ),
            );
          }
          // else → shop auto removed
        }
      ...
      state = CartState(isLoading: false, cartItems: cartItems);
      debugPrint("Delete cart error: $error");
    }
  }
